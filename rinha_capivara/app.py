...
async def fazer_transacao(cliente_id: int, transacao: TransacaoReq):
    ...
    return SaldoCliente(limite=limite_cliente_long, saldo=saldo_atualizado_long)


@app.get('/clientes/{cliente_id}/extrato')
async def get_cliente_extrato(cliente_id: int):
    with get_db() as session:
    ...
    return Extrato(saldo=saldo, ultimas_transacoes=ultimas_transacoes)
